...
router.get("/find/:placename", async (req : Request, res : Response) => {
    try{
        ...
        const response = await axios.get(`https://api.geoapify.com/v1/geocode/autocomplete?text==${req.params.placename}&apiKey=${process.env.GEOAPIFY_SECRET}&format=json&limit=10&lang=id`)
        if(response){
...
router.get("/reverse/lat=:lat&lon=:lon", async (req : Request, res : Response) => {
    try{
        const response = await axios.get(`https://api.geoapify.com/v1/geocode/reverse?lat=${req.params.lat}&lon=${req.params.lon}&apiKey=${process.env.GEOAPIFY_SECRET}`)
        if(response){
